...
function TeamMember({ name, role, imgPlaceholder = true, index = 0, linkedin, image }: TeamMemberProps) {
  return (
    <motion.div
      className="flex flex-col items-center h-full text-center px-2"
    ...
    >
      ...
      {linkedin && (
        <a
          ...
          className="text-blue-600 hover:text-blue-800 mt-auto pt-2"
        >
          <FaLinkedin size={20} />
        </a>
      )}
...
function TeamSection2({ title, members }: TeamSectionProps) {
  const [isExpanded, setIsExpanded] = useState(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedComponent animation="fadeIn" delay={0.4} duration={0.4}>
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <TeamSection2 title={t("about.board")} members={advisoryBoard} />
          </motion.div>
        </AnimatedComponent>
